...
def uploadData(upload_file, port, table_name='default', log_path='C:/Users/zhan1/Desktop/Python/project_jack/corona/', addData=False):
    try:
        connection = psycopg2.connect(user='postgres', 
                                            password='qwertyuiop135', 
                                            host='127.0.0.1', 
    ...
    if upload_file.endswith('csv'):
        ...
        if addData and table_name != 'default':
            ...
            if not all(elem in columns_list for elem in existing_column):
                ...
                try:
                    ...
                    goLog(log_path, 'PostgresSQL_uploadData: Failed to add to table', level='error')
                    quit

        elif table_name == 'default' and addData:
            raise KeyError('Have to specify the table if you want to add data.')
            ...
            try:
                ...
                goLog(log_path, 'PostgresSQL_uploadData: Failed to create table.', level='error')
                quit
